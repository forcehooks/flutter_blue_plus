...
public class FlutterBluePlusPlugin implements FlutterPlugin, MethodCallHandler, RequestPermissionsResultListener, ActivityAware {
        ...
        (granted, perms) -> {
          if (granted) {
            ...
            result.error("no_permissions",
              String.format("flutter_blue plugin requires %s for scanning",
                perms), null);
          }
        }
      );
    ...
    case "getConnectedDevices": {
      ensurePermissionsBeforeAction(
        ...
        (granted, permission) -> {
          if (!granted) {
            result.error("no_permissions",
              String.format("flutter_blue plugin requires %s for obtaining connected devices",
                permission), null);
            return;
          }
    ...
    case "connect": {
      ensurePermissionsBeforeAction(
        ...
        (granted, permission) -> {
          if (!granted) {
            result.error("no_permissions",
              String.format("flutter_blue plugin requires %s for new connection",
                permission), null);
            return;
          }
  ...
  private void ensurePermissionsBeforeAction(String[] permissions, OperationOnPermission operation) {
    ...
    if (allPermissionsGranted) {
      ...
      final int requestCode = lastEventId;
      operationsOnPermission.put(requestCode, (granted, perms) -> {
        operationsOnPermission.remove(requestCode);
        operation.op(granted, perms);
      });
    ...
    public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
      ...
      if (newState == 2) {
        if (gatt.requestMtu(512)) {
          log(LogLevel.DEBUG, "[requestMtu] status");
        } else {
          log(LogLevel.DEBUG, "gatt.requestMtu returned false");
        }
      }
